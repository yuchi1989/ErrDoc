/*
 * Collecting function calls lists along each error path for function pairs analysis
 */
...
//before building this checker using clang, change the following macro value to absolute path of "fp_analysis/openssl_error_spec.txt"
...
/* the RPEx checker */
class RPEx : public Checker<check::PreCall, check::PostCall, check::EndFunction,
...
size_t RPEx::parseOnce(char *buf)
{
	size_t count = 0;
	char *tok = NULL;
	char *func_name = NULL;
...
void RPEx::checkPostCall(const CallEvent &Call, CheckerContext &C) const
{
...
bool RPEx::getConcreteValue(SVal val, int64_t *ret) const
{
...
void RPEx::checkPreCall(const CallEvent &Call, CheckerContext &C) const
{
...
void RPEx::printMsg(std::string str) const {
	llvm::errs() << "RPEx: " << str << "\n";
}

void RPEx::checkPreStmt(const ReturnStmt *ret_stmt, CheckerContext &C) const
{
...
void RPEx::checkEndFunction(CheckerContext &C) const
{
	const clang::Decl *DC = C.getCurrentAnalysisDeclContext()->getDecl();
	std::string caller = DC->getAsFunction()->getNameInfo().getAsString();
	ProgramStateRef state = C.getState();
...
ProgramStateRef
RPEx::isError(enum IsError *isErrorPathOut, StringRef name, SVal ret,
 ...
 */
void ento::registerRPEx(CheckerManager &mgr)
{
	mgr.registerChecker<RPEx>();
}
